...
import { checklistSchema, type ChecklistFormValues } from "./schemas/checklist-schema"
...
export function ChecklistForm({ onNext, onPrevious, applicationData, continueButtonClass, isStepCompleted }: ChecklistFormProps) {
  ...
  const isComplete = {
    ...
    currentAddress: () => applicationData?.residentialInfo?.address && applicationData?.residentialInfo?.addressStatus,
    prevAddresses: () => applicationData?.residentialInfo?.prevAddresses?.length > 0,
